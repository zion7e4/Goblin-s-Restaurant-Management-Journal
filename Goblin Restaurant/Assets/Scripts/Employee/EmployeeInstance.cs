...
/// <summary>
/// 직원의 모든 데이터를 담고 관리하는 데이터 클래스입니다.
/// 이 클래스의 인스턴스는 직원 한 명의 데이터를 갖습니다.
/// </summary>
[System.Serializable]
public class EmployeeInstance
{
    ...
    /// <summary>
    /// 이 직원의 바탕이 되는 직원 데이터(ScriptableObject)입니다. 변하지 않는 기본 정보를 담고 있습니다.
    /// </summary>
    public EmployeeData BaseData { get; private set; }

    /// <summary>
    /// 이 직원이 주인공인지 여부입니다. (해고 불가)
    /// </summary>
    public bool isProtagonist { get; private set; }

    /// <summary>
    /// 게임 내 플레이어가 정하는 이름입니다.
    /// </summary>
    ...
    /// <summary>
    /// '지원자(GeneratedApplicant)' 데이터로부터 새로운 직원 인스턴스를 생성합니다. (일반 직원)
    /// </summary>
    ...
    /// <summary>
    /// '주인공'처럼 특정 템플릿(EmployeeData)에서 직원 인스턴스를 생성합니다.
    /// </summary>
    public EmployeeInstance(EmployeeData baseData)
    {
        BaseData = baseData;
        // 플레이어가 직접 정하기 전의 기본 이름 설정
        firstName = "Goblin Chef";
        currentLevel = 1;
        currentExperience = 0;
        skillPoints = 5; // 주인공은 기본 스킬 포인트 보유 (설정)
        ...
        // baseData(mainCharacterTemplate)에 있는 특성("주인공")을
        // 이 직원의 현재 특성(currentTraits) 리스트에 추가합니다.
    ...
    // ************* [능력치 강화 함수는 아직 구현되지 않았으므로 보류] *************

    /// <summary>
    /// 요리 스탯에 스킬 포인트를 사용하고 스탯을 업데이트합니다.
    /// </summary>
    /// <returns>스탯 사용이 성공했으면 true를 반환합니다.</returns>
    public bool SpendSkillPointOnCooking()
    {
        if (skillPoints > 0)
        {
            skillPoints--;
            currentCookingStat++;
            Debug.Log($"{firstName}: 요리 스탯이 {currentCookingStat}으로 올랐습니다. 남은 포인트: {skillPoints}");
            return true;
        }
    ...
    /// <summary>
    /// 서빙 스탯에 스킬 포인트를 사용하고 스탯을 업데이트합니다.
    /// </summary>
    public bool SpendSkillPointOnServing()
    {
        if (skillPoints > 0)
        {
            skillPoints--;
            currentServingStat++;
            Debug.Log($"{firstName}: 서빙 스탯이 {currentServingStat}으로 올랐습니다. 남은 포인트: {skillPoints}");
            return true;
        }
    ...
    /// <summary>
    /// 매력 스탯에 스킬 포인트를 사용하고 스탯을 업데이트합니다.
    /// </summary>
    public bool SpendSkillPointOnCharm()
    {
        if (skillPoints > 0)
        {
            skillPoints--;
            currentCharmStat++;
            Debug.Log($"{firstName}: 매력 스탯이 {currentCharmStat}으로 올랐습니다. 남은 포인트: {skillPoints}");
            return true;
        }
    ...
    /// <summary>
    /// 이 직원이 가진 모든 특성들의 '재료 절약' 확률을 합산하여 반환합니다.
    /// </summary>
    ...
    {
        // 1. 등급(Grade)에 따른 최대 레벨을 설정합니다. (기획서 내용)
        int maxLevel;
        switch (this.grade)
        {
        ...
        // 2. 골드 소모 확인 (기획서 내용)
        // (기획서 내용: 레벨업 필요 골드 = 이전 필요 골드 * 1.1)
        int requiredGold = (int)(100 * Mathf.Pow(1.1f, currentLevel - 1)); // (기획서 10% 복리 계산 임시 공식)
        ...
        currentLevel++;
        skillPoints++; // ㅋㅋㅋ 기획서에는 SP 1 지급 ㅋㅋㅋ

        Debug.Log($"[레벨업!] {firstName} (Lv. {currentLevel}), SP +1. (소모: {requiredGold}G)");
        return true;
    }
    /// <summary>
    /// 이 직원이 가진 모든 특성들의 '재료 훔치기' 확률을 합산하여 반환합니다.
    /// </summary>
    ...
    /// <summary>
    /// 이 직원이 가진 모든 특성들의 '요리 속도 보너스' 배율을 합산하여 반환합니다.
    /// </summary>
    ...
    /// <summary>
    /// 이 직원이 가진 모든 특성들의 '이동 속도 보너스' 배율을 합산하여 반환합니다.
    /// </summary>
    ...
    /// <summary>
    /// 이 직원이 가진 모든 특성들의 '작업 속도 보너스' 배율을 합산하여 반환합니다.
    /// </summary>
    ...
    /// <summary>
    /// 이 직원이 가진 모든 특성들의 '서비스 점수' 보너스/패널티를 합산하여 반환합니다.
    /// </summary>
    public int GetTraitServiceScoreBonus()
    {
        ...
        // "진상손님"(-5)과 "친절함"(+?) 같은 특성의 값을 합산
        return currentTraits.Sum(trait => trait.serviceScoreModifier);
    }
    /// <summary>
    /// 이 직원이 가진 모든 특성들의 '팁 확률' 보너스를 합산하여 반환합니다.
    /// </summary>
    ...
    /// <summary>
    /// 이 직원이 가진 모든 특성들의 '모든 스탯 보너스' 배율을 합산하여 반환합니다.
    /// </summary>
